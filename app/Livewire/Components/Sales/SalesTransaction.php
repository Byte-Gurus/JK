...
class SalesTransaction extends Component
{
        ...
        if ($itemData) {
            ...
            if (!$itemExists) {
                $this->selectedItems[] = [
                    ...
                    'item_unit' => $item->itemJoin->item_unit,
                    'item_category' => $item->itemJoin->item_category,
    ...
    public function computeTransaction()
    {
        ...
        foreach ($this->selectedItems as $index) {
            ...
            if ($index['vat_type'] === 'Vat') {
                ...
                $vat_exempt_amount = (($vat_exempt_subtotal * (100 + $index['vat_percent'])) / 100) - $vat_exempt_subtotal;
    ...
    public function save()
    {
        ...
        try {
            ...
            $log = Log::create([
                ...
                'message' => $userName . ' (' . Auth::user()->username . ') ' . 'Created a transaction',
    ...
    {
        $this->dispatch('barcode_focus')->to(SalesTransaction::class);
    }
        ...
        foreach ($returnDetails as $index => $returnDetail) {
            if ($returnDetail->operation === 'Exchange') {
                ...
                $selectedItem = [
                    ...
                    'item_unit' => $returnDetail->transactionDetailsJoin->itemJoin->item_unit,
                    'item_category' => $returnDetail->transactionDetailsJoin->itemJoin->item_category,
